...
export async function POST(_request: Request, context: RouteContext) {
  ...
  const apiResponse = await fetch(`${API_BASE_URL}/company-operations/employees/${employeeId}/archive`, {
    method: "POST",
    headers: {
  ...
  if (!apiResponse.ok) {
    return NextResponse.json(
      { message: formatApiMessage(responsePayload, "Unable to deactivate employee.") },
