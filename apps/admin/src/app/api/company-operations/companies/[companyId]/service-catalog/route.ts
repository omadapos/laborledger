...
export async function POST(request: Request, context: RouteContext) {
  ...
  const body = (await request.json().catch(() => null)) as CreateServiceCatalogItemBody | null;
  ...
  const apiResponse = await fetch(`${API_BASE_URL}/company-operations/companies/${companyId}/service-catalog`, {
    method: "POST",
    headers: {
  ...
  if (!apiResponse.ok) {
    return NextResponse.json(
      { message: formatApiMessage(responsePayload, "Unable to create service catalog item.") },
