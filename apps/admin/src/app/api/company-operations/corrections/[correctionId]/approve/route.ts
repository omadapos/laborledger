...
export async function POST(request: Request, context: RouteContext) {
  ...
  const apiResponse = await fetch(`${API_BASE_URL}/company-operations/corrections/${correctionId}/approve`, {
    method: "POST",
  ...
  if (!apiResponse.ok) {
    return NextResponse.json({ message: formatApiMessage(payload, "Unable to approve correction.") }, { status: apiResponse.status });
  }
