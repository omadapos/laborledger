...
import { CorrectionStatus, PrismaClient } from "@prisma/client";
...
describe("corrections workflow", () => {
  ...
  it("creates, approves, applies a missing clock-out correction and keeps original punch events", async () => {
    ...
    const originalCount = await prisma.punchEvent.count({ where: { shiftId: shift.id } });

    const createResponse = await request(httpServer)
      .post(`/company-operations/shifts/${shift.id}/corrections`)
      .set("Cookie", fixture.adminSession)
      .send({
        type: "MISSING_CLOCK_OUT",
        reason: "Employee forgot to clock out at kiosk.",
        proposedEventUtc: "2026-04-06T21:00:00.000Z"
      })
      .expect(201);

    expect(createResponse.body.status).toBe(CorrectionStatus.PENDING);

    const correctionId = createResponse.body.id as string;

    await request(httpServer)
      .post(`/company-operations/corrections/${correctionId}/approve`)
      .set("Cookie", fixture.adminSession)
      .send({})
      .expect(201);

    const applyResponse = await request(httpServer)
      .post(`/company-operations/corrections/${correctionId}/apply`)
      .set("Cookie", fixture.adminSession)
      .expect(201);

    expect(applyResponse.body.status).toBe(CorrectionStatus.APPLIED);
    expect(await prisma.punchEvent.count({ where: { shiftId: shift.id } })).toBe(originalCount);

    const reviewList = await request(httpServer)
      .get(`/company-operations/companies/${fixture.companyId}/review-shifts`)
      .set("Cookie", fixture.adminSession)
      .expect(200);

    expect(reviewList.body[0].punchState).toBe("clocked_out");
    expect(reviewList.body[0].workedMinutes).toBe(450);
  });

  it("rejects correction without reason and blocks applying rejected corrections", async () => {
    const fixture = await createFixture();
    ...
    const createResponse = await request(httpServer)
      ...
      .send({
        type: "MISSING_CLOCK_OUT",
        reason: "Will reject this",
        proposedEventUtc: "2026-04-06T21:00:00.000Z"
      })
      .expect(201);

    await request(httpServer)
      .post(`/company-operations/corrections/${createResponse.body.id}/reject`)
      .set("Cookie", fixture.adminSession)
      .send({ reviewReason: "Not supported" })
      .expect(201);

    await request(httpServer)
      .post(`/company-operations/corrections/${createResponse.body.id}/apply`)
      .set("Cookie", fixture.adminSession)
      .expect(400);
  });

  it("rejects cross-company correction access", async () => {
    const alpha = await createFixture("Alpha");
    const beta = await createFixture("Beta");

    await request(httpServer)
      .get(`/company-operations/companies/${alpha.companyId}/corrections`)
      .set("Cookie", beta.adminSession)
      .expect(403);
  });

  async function createFixture(label = "Alpha") {
    ...
    const locationResponse = await request(httpServer)
      ...
      .set("Cookie", adminSession)
      .send({ name: `${label} Site`, timezone: "America/New_York", serviceClientId: clientResponse.body.id })
      .expect(201);
    ...
    const employee = await prisma.employee.findFirstOrThrow({ where: { companyId: setup.companyId } });
    ...
    return { companyId: setup.companyId, adminSession, kiosk, kioskSecret };
  }
  ...
  async function createCompanyWithAdmin(superadminSession: string, groupLabel: string, adminPrefix: string) {
    ...
    await request(httpServer)
      ...
      .send({ token: groupResponse.body.invitationToken, password: ownerPassword, fullName: `${groupLabel} Owner` })
      .expect(200);
    ...
    await request(httpServer)
      .post("/invitations/accept")
      .send({ token: companyResponse.body.invitationToken, password: adminPassword, fullName: `${groupLabel} Admin` })
      .expect(200);
    return { companyId: companyResponse.body.company.id as string, adminEmail, adminPassword };
  }
